...
module.exports = {
    async up(queryInterface, Sequelize) {
        await queryInterface.createTable("m_kategori_lembur", {
        ...
        await queryInterface.createTable("m_jenis_pekerjaan_lembur", {
        ...
        ].map(([kode_jenis, nama_jenis, category, kode_perilaku, urutan]) => ({ id_kategori_lembur: categoryId[category], kode_jenis, nama_jenis, kode_perilaku, requires_replacement_officer: kode_perilaku === "PENGGANTI_KETIDAKHADIRAN" ? "Y" : "N", evidence_optional: kode_perilaku === "REGULAR" ? "N" : "Y", max_daily_hours: kode_perilaku === "REGULAR" ? 4 : 8, urutan, is_active: "Y", created_at: now })));
        await queryInterface.sequelize.query("UPDATE t_spkl s JOIN m_kategori_lembur k ON CONVERT(k.nama_kategori USING utf8mb4) COLLATE utf8mb4_uca1400_ai_ci = CONVERT(s.kategori_lembur USING utf8mb4) COLLATE utf8mb4_uca1400_ai_ci SET s.id_kategori_lembur = k.id_kategori_lembur");
        await queryInterface.sequelize.query("UPDATE t_spkl s JOIN m_jenis_pekerjaan_lembur j ON CONVERT(j.nama_jenis USING utf8mb4) COLLATE utf8mb4_uca1400_ai_ci = CONVERT(s.jenis_pekerjaan USING utf8mb4) COLLATE utf8mb4_uca1400_ai_ci AND j.id_kategori_lembur = s.id_kategori_lembur SET s.id_jenis_pekerjaan_lembur = j.id_jenis_pekerjaan_lembur");
        await queryInterface.sequelize.query("UPDATE t_lembur l JOIN m_kategori_lembur k ON CONVERT(k.nama_kategori USING utf8mb4) COLLATE utf8mb4_uca1400_ai_ci = CONVERT(l.kategori_lembur USING utf8mb4) COLLATE utf8mb4_uca1400_ai_ci SET l.id_kategori_lembur = k.id_kategori_lembur");
        await queryInterface.sequelize.query("UPDATE t_lembur l JOIN m_jenis_pekerjaan_lembur j ON CONVERT(j.nama_jenis USING utf8mb4) COLLATE utf8mb4_uca1400_ai_ci = CONVERT(l.jenis_pekerjaan USING utf8mb4) COLLATE utf8mb4_uca1400_ai_ci AND j.id_kategori_lembur = l.id_kategori_lembur SET l.id_jenis_pekerjaan_lembur = j.id_jenis_pekerjaan_lembur");
    },
