...
module.exports = {
    async up(queryInterface, Sequelize) {
        let petugasColumns = await queryInterface.describeTable("m_petugas");
        ...
        if (!petugasColumns.id_project) {
            ...
            petugasColumns = await queryInterface.describeTable("m_petugas");
        }
        ...
        const indexes = await queryInterface.showIndex("m_petugas");
        if (!indexes.some((index) => index.name === "idx_petugas_project")) {
            await queryInterface.addIndex("m_petugas", ["id_project"], { name: "idx_petugas_project" });
        }
        if (jabatanColumns.id_project) await queryInterface.removeColumn("m_jabatan", "id_project");
    },
