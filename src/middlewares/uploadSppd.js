...
const storage = multer.diskStorage({
    destination: (req, file, callback) => {
        const directory = path.join(process.cwd(), "uploads", "sppd", getRelativeDirectory(file.fieldname));
...
module.exports = multer({
    ...
    fileFilter: (req, file, callback) => {
        ...
        if (!allowed.includes(file.mimetype)) {
            return callback(new Error("Signature harus berformat JPG, JPEG, PNG, atau WEBP"), false);
        }
        callback(null, true);
    },
